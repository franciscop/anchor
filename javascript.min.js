...
const recordar=(tries=[])=>new Promise((resolve,reject)=>{  if(!(tries instanceof Array)){    throw new Error('The parameter should be an array of tries');
 }  if(tries.length===0){    return resolve(0.5);
 }
  Object.keys(recordar.factors).reduce((all,key)=>{    return all.then(glo=>recordar.factors[key](tries).then(local=>{      return glo * local * 2;
   }));
...
recordar.factors.forget=tries=>new Promise((resolve,reject)=>{  if(!tries.length) return resolve(0.5);
...
recordar.factors.accuracy=tries=>new Promise((resolve,reject)=>{  if(!tries.length) return resolve(1);
...
if(typeof module !=='undefined'){  module.exports=recordar}
function touch(cb){  var start;
  ...
  document.addEventListener('touchmove',function(e){    if(!e.changedTouches.length) return;
...
let memoriza;
if(typeof module !=='undefined'){  memoriza=require('./memory/index.js')}
...
var FlashCard=function(selector,opts){  if(!(this instanceof FlashCard)){    return new FlashCard(selector,opts);
  ...
  touch(diff=>{    if(diff.final){      for(let i=0; i <=9; i++){        this.element.classList.remove('p-'+i);
     }      this.element.classList.remove('move','left','right');
      this.element.classList.add('final');
      this.element.style.left='0px';
      this.element.style.top='0px';
   }else{      for(let i=0; i <=9; i++){        this.element.classList.remove('p-'+i);
      ...
      if(diff.x < 0){        this.element.classList.add('left');
     }else{        this.element.classList.add('right');
     }      if(Math.abs(diff.x) < 100)
        this.element.classList.add('p-'+parseInt(Math.abs(diff.x / 10)));
   }
    if(diff.x < -100 && Math.abs(diff.x / diff.y) > 1){      this.element.classList.add('left');
      if(this.left && diff.final) this.left();
   }    if(diff.x > 100 && Math.abs(diff.x / diff.y) > 1){      this.element.classList.add('right');
      if(this.right && diff.final) this.right();
   }    if(diff.y < -100 && Math.abs(diff.x / diff.y) > 1){      this.element.classList.add('up');
      if(this.up && diff.final) this.up();
   }    if(diff.y > 100 && Math.abs(diff.x / diff.y) > 1){      this.element.classList.add('down');
      if(this.down && diff.final) this.down();
   }    if(Math.abs(diff.y) < 100 && Math.abs(diff.x) < 100){      if(diff.final) this.tap();
   } });
...
function process(word){  return{    id:word.id,    index:word.index,
    ...
    `).join(''),    titlebottomright:'Time from the last try at this card',    bottomright:word.tries.slice(-1).map(tried=>moment ?
      moment(tried.time).fromNow():'').join(''),
    top:word.extra || '',    middle:word.word,    bottom:word.meaning
 }}
...
var display=(initial)=>memory.pick().then(word=>{  word=initial && initial.id ? initial:word;
  ...
  if(word.tries && word.tries.length===0){    card.flip();
 }
